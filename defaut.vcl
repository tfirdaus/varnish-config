...
acl purge {
	"localhost";
	"127.0.0.1";
	"0.0.0.0"; # Add custom IP if needed
}
...
sub vcl_recv {
	...
	if (req.url ~ "nocache|wp-admin|sign-in|wp-(comments-post|login|signup|activate|mail|cron)\.php|preview\=true|admin-ajax\.php|xmlrpc\.php|bb-admin|server-status|control\.php|bb-login\.php|bb-reset-password\.php|register\.php") {
		return(pass);
	}
...
sub vcl_backend_response {
	# Don't store backend
	if (bereq.url ~ "nocache|wp-admin|admin|login|sign-in|wp-(comments-post|login|signup|activate|mail|cron)\.php|preview\=true|admin-ajax\.php|xmlrpc\.php|bb-admin|server-status|control\.php|bb-login\.php|bb-reset-password\.php|register\.php") {
		set beresp.uncacheable = true;
		return (deliver);
	}
	# Otherwise cache away!
	if ( (!(bereq.url ~ "nocache|wp-admin|admin|login|sign-in|wp-(comments-post|login|signup|activate|mail|cron)\.php|preview\=true|admin-ajax\.php|xmlrpc\.php|bb-admin|server-status|control\.php|bb-login\.php|bb-reset-password\.php|register\.php")) || (bereq.method == "GET") ) {
